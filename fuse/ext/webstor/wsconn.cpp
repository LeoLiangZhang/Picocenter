...
// liang: add this for number conversion
#include <sstream>
...
// static const UInt32 s_defaultTimeout = 300 * 1000;          // 5 mins // liang:changed
    ...
    try
    {
        ...
        if( startsWith( p, size, STRING_WITH_LEN( "HTTP" ), &prefixLen ) )
        {
            ...
            if( startsWith( p, size, STRING_WITH_LEN( "200 OK" ) ) || 
                ...
                startsWith( p, size, STRING_WITH_LEN( "206 Partial Content" ) ) ) // liang: for getRange 
            {
...
static void
completeGetRange( WsResponseDetails &responseDetails, WsGetResponse *response )
{
    // A special case for GetResponse: treat NoSuchKey as success
    // but with loadedContentLength = -1.
    ...
    if ( responseDetails.status == WS_RESPONSE_STATUS_HTTP_FAILURE && 
         startsWith(responseDetails.httpStatus.c_str(), "416") )
    {// expecting httpStatus: "416 Requested Range Not Satisfiable"
        responseDetails.status = WS_RESPONSE_STATUS_SUCCESS;
        responseDetails.loadedContentLength = 0;
    }
...
// liang: testing code for range get
void
WsConnection::getRange( const char *bucketName, const char *key, 
                    ...
                    WsGetResponse *response /* out */ )
{
    ...
    try
    {
        ...
        // liang: add range header if user reqeusted
        if (rangeStart || rangeSize)
        {
            std::stringstream range;
            range << "bytes=" << rangeStart << "-";
            if (rangeSize){
                size_t rangeEnd = rangeStart+rangeSize-1; // range is inclusive
                range << rangeEnd;
            }
            appendRequestHeader("Range", range.str().c_str(), &request.headers);
        }
        init( &request, bucketName, key );
        ...
        ::webstor::completeGetRange( responseDetails, response );
    }
    catch( ... )
    {
        throwSummary( "get", key );
...
void
WsConnection::getRange( const char *bucketName, const char *key, void *buffer, size_t size, 
                    size_t rangeStart, size_t rangeSize,
                    WsGetResponse *response /* out */ )
{
    dbgAssert( bucketName );
    ...
    WsGetResponseBufferLoader loader( buffer, size );
    getRange( bucketName, key, rangeStart, rangeSize, &loader, response );
}
